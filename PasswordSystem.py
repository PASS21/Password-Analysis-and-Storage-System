...
def writeLogins(masterKeyInput):
    ...
    if masterKeyInput==actualMasterKey:
        ...
        import sys
        from getpass import getpass
        nm = str(input("Enter website name: "))
        url = str(input("Enter website url: "))
        usr = str(input("Enter username: "))
        pwd = str(getpass("Enter password: "))
        logins=[]
        logins += [nm,url,usr,pwd]
        ...
        if check(pwd)<1.5 and (strength(pwd)>=25 or eval_time(pwd)>=3600) :
            ...
            if ch=="y" or ch=="":
                pwd = str(passgen()) # made it generate only 1 password - 1 is the default argument for passgen()
                del logins[-1]
                logins.append(pwd)
                csvWriter = csv.writer(csvFile)
                csvWriter.writerow(logins)
...
|____/ \__, |___/\__\___|_| |_| |_|
    ...
    Note: You can view this menu again by typing Enter.
    """
    )
...
def main_func():
    ...
    if str(choice)=="4":
        ...
        if os.path.isfile("./Passwds.csv"):
            ...
            print("Done.")
            print("Authenticate yourself !  ")
            imk = getpass("Enter master key: ") #no need to ask it again ???
            editLogins(imk)
            print("Logins edited successfully !")
